...
function Play() {
  ...
  const logInView = () => {
    return <PlayForm logIn={logIn} noRoom={noRoom} />;
  };

  const loggedInView = () => {
    return <WaitingForOthers name={playerName} />;
  };
    ...
    switch (testType) {
      ...
      case "image":
        return (
          <AnswersPromptImage
            images={images}
            questions={currentAnswerOptions}
            sendAnswer={sendAnswer}
          />
        );
      default:
